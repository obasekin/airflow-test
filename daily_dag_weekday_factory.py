...
CONFIG_PATH = Path(__file__).with_name("daily_dag_weekday_config.yaml")


def _country_config(country):
    config = yaml.safe_load(CONFIG_PATH.read_text(encoding="utf-8"))
    return config["countries"][country]


def register_country_dag(country):
    settings = _country_config(country)
    local_tz = pendulum.timezone(settings["timezone"])
    gcs_bucket_name = settings["bucket"]
    gcs_base_path = settings["paths"]["gcs_base"].format(country=country)
    manifest_prefixes = settings["manifest_prefixes"]
    failure_email = EmailNotifier(to_email=settings["email"])
    airflow_home = os.environ.get("AIRFLOW_HOME", "/opt/airflow")
    ingestion_spec = (
        Path(airflow_home)
        / "dags"
        / "repo"
        / settings["paths"]["ingestion_spec"].format(country=country)
    )
    metadata = settings["metadata"]
    default_args = {
        "owner": metadata["owner"],
        "retries": metadata["retries"],
        "retry_delay": timedelta(minutes=metadata["retry_delay_minutes"]),
    ...
    @dag(
        ...
        start_date=pendulum.datetime(
            *(int(part) for part in metadata["start_date"].split("-")),
            tz=local_tz,
        ),
    ...
    def druid_ingestion_workflow():
        ...
        return druid_ingestion_workflow()


# Register defaults only when a country does not have its own DAG module. A file
# named `<COUNTRY>_daily_dag_weekday.py` owns that country completely and may
# either define a custom DAG or call register_country_dag() for the default.
_config = yaml.safe_load(CONFIG_PATH.read_text(encoding="utf-8"))
for _country in _config["countries"]:
    _country_module = Path(__file__).with_name(
        f"{_country}_daily_dag_weekday.py"
    )
    if not _country_module.exists():
        globals()[f"{_country}_daily_dag_weekday"] = register_country_dag(
            _country
        )
